...
import { Calendar, Clock, Trash2, AlertCircle, Loader2, HandHelping, X, CheckCircle2, Pencil, Save, Settings, ArrowLeft, Archive, Repeat, Plus, Users, ListTodo, BarChart3, ArrowLeftRight, Utensils, Info, Eye, EyeOff, Filter } from "lucide-react";
import {
  signUp,
  signIn,
  signOut,
...
import {
  ...
  setHoursAdjustment,
  loadFreelanceAbsences,
  addFreelanceAbsence,
  deleteFreelanceAbsence,
  loadTeamRoster,
  addTeamMember,
  removeTeamMember,
  setTeamMemberFreelance,
} from "./dataLayer";
...
// Correspondance entre chaque page de l'app et son adresse dans le navigateur, pour que
// chaque onglet ait sa propre URL (favoris, bouton précédent/suivant, lien partageable).
const PAGE_PATHS = {
  admin: "/administration",
  planning: "/planning",
  main: "/absences",
  freelance: "/repertoire-freelances",
  echanges: "/echanges",
  archive: "/archives",
};
const PATH_TO_PAGE = Object.fromEntries(Object.entries(PAGE_PATHS).map(([k, v]) => [v, k]));

// Portions de créneaux, par jour et par personne, à afficher en gris dans le Planning car
// elles ne correspondent pas à du temps de traitement des tickets (le reste du créneau, non
// listé ici, reste affiché en violet). Une entrée [0, 1440] couvre toute la journée.

const NON_TICKET_ZONES = {
  Lundi: {
    Capucine: [[0, 1440]],
    Sylvain: [[0, 1440]],
  },
...
// Découpe un créneau [start,end) en sous-parties violettes (tickets) / grises (hors tickets),
// selon les zones définies ci-dessus pour ce jour et cette personne.
function splitSegmentByTicketZones(jour, prenom, start, end) {
  const zones = (NON_TICKET_ZONES[jour] || {})[prenom];
  if (!zones) return [{ start, end, isGrey: false }];
  // Fusionne les zones grises qui se chevauchent avec le créneau, dans l'ordre.
  const greyRanges = zones
    .map(([zs, ze]) => [Math.max(start, zs), Math.min(end, ze)])
    .filter(([zs, ze]) => zs < ze)
    .sort((a, b) => a[0] - b[0]);

  const chunks = [];
  let cursor = start;
  for (const [zs, ze] of greyRanges) {
    if (zs > cursor) chunks.push({ start: cursor, end: zs, isGrey: false });
    chunks.push({ start: Math.max(zs, cursor), end: ze, isGrey: true });
    cursor = Math.max(cursor, ze);
  }
  if (cursor < end) chunks.push({ start: cursor, end, isGrey: false });
  return chunks.filter((c) => c.end > c.start);
}

// Affiche simplement le prénom (le statut freelance est indiqué par une case à cocher
// dédiée dans "Heures travaillées par personne", plus par ce suffixe).
function displayPersonName(prenom) {
  return prenom;
}
...
// Arrondit à l'heure la plus proche (ex. 1h59 -> 2h, 2h01 -> 2h), utilisé dans "Heures
// travaillées par personne" et "Heures supplémentaires par personne".
function formatDurationRounded(minutes) {
...
// Champ prénom avec suggestions : filtre la liste des prénoms connus au fur et à mesure de la
// saisie (insensible à la casse/aux accents), tout en restant un champ texte librement modifiable.
function normalizeNameForAutocomplete(s) {
  return (s || "").trim().toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
}
function PrenomAutocomplete({ value, onChange, options, invalid, className }) {
  const [open, setOpen] = useState(false);
  const norm = normalizeNameForAutocomplete(value);
  const filtered = norm ? options.filter((o) => normalizeNameForAutocomplete(o).includes(norm)) : options;
  ...
  return (
    <div className="relative">
      <input
        ...
        className={
          className ||
          `w-full px-3 py-2.5 rounded-lg border focus:outline-none focus:ring-2 text-sm ${
            invalid
          ...
          {filtered.map((name) => (
            <li key={name}>
              <button
                ...
                className="w-full text-left px-3 py-2 text-sm text-slate-700 hover:bg-violet-50"
              >
                {name}
              </button>
            </li>
...
export default function AbsenceTracker() {
  ...
  const [mainYearFilter, setMainYearFilter] = useState(new Date().getFullYear());
  const [mainMonthFilter, setMainMonthFilter] = useState("toutes");

  // navigation : page principale ou page d'administration
  const [page, setPage] = useState(() => PATH_TO_PAGE[window.location.pathname] || "main");

  // Garde l'adresse du navigateur synchronisée avec la page affichée.
  useEffect(() => {
  ...
  const [majoreesYearFilter, setMajoreesYearFilter] = useState(today.getFullYear());
  const [majoreesMonthFilter, setMajoreesMonthFilter] = useState(today.getMonth() + 1);
  const [yearFilter, setYearFilter] = useState(today.getFullYear());
  const [monthFilter, setMonthFilter] = useState(today.getMonth() + 1);
  const [archiveYearFilter, setArchiveYearFilter] = useState(today.getFullYear());
  const [archiveMonthFilter, setArchiveMonthFilter] = useState("toutes");
  const [freelanceYearFilter, setFreelanceYearFilter] = useState(today.getFullYear());
  const [freelanceMonthFilter, setFreelanceMonthFilter] = useState("toutes");
  const [archiveExchangesYearFilter, setArchiveExchangesYearFilter] = useState(today.getFullYear());
  const [archiveExchangesMonthFilter, setArchiveExchangesMonthFilter] = useState("toutes");
  const [archivedAbsencesVisibleCount, setArchivedAbsencesVisibleCount] = useState(10);
  const [archivedExchangesVisibleCount, setArchivedExchangesVisibleCount] = useState(10);
  ...
  // Mémorise, une fois pour toutes au chargement, si on est dans le parcours "mot de
  // passe oublié" — et ignore ENSUITE tous les événements de connexion (Supabase peut
  // en déclencher plusieurs à la suite pour la même session : PASSWORD_RECOVERY, puis
  // SIGNED_IN, TOKEN_REFRESHED...) jusqu'à ce que la personne ait vraiment changé son
  // mot de passe. C'est ce qui empêche d'être renvoyé·e vers la plateforme trop tôt.
  const isRecoveryFlowRef = useRef(window.location.hash.includes("type=recovery"));
  ...
  // ---------------------------------------------------------------------
  // Répertoire des absences (réservé aux Freelances) : liste simple, sans
  // système de remplacement/validation.
  // ---------------------------------------------------------------------
  const [freelanceAbsences, setFreelanceAbsences] = useState(null); // null = chargement
  const [freelancePrenom, setFreelancePrenom] = useState("");
  const [freelanceDate, setFreelanceDate] = useState("");
  const [freelanceHeureDebut, setFreelanceHeureDebut] = useState("");
  const [freelanceHeureFin, setFreelanceHeureFin] = useState("");
  const [freelanceFormError, setFreelanceFormError] = useState("");
  const [freelanceInvalidFields, setFreelanceInvalidFields] = useState({});
  const [freelanceSaving, setFreelanceSaving] = useState(false);
  const [freelanceError, setFreelanceError] = useState("");

  // Liste de l'équipe modifiable (remplace la liste figée dans le code) : utilisée par
  // "Heures travaillées par personne" et par les suggestions de saisie du prénom.
  const [teamRoster, setTeamRoster] = useState([]); // [{ prenom, isFreelance }]
  const [newTeamMemberName, setNewTeamMemberName] = useState("");
  const [teamRosterError, setTeamRosterError] = useState("");

  const teamRosterNames = useMemo(() => teamRoster.map((r) => r.prenom), [teamRoster]);
  const freelanceNamesSet = useMemo(
    () => new Set(teamRoster.filter((r) => r.isFreelance).map((r) => r.prenom)),
    [teamRoster]
  );

  async function refreshTeamRoster() {
    try {
      const data = await loadTeamRoster();
      setTeamRoster(data);
      setTeamRosterError("");
    } catch (e) {
      console.error("Erreur de chargement de l'équipe:", e);
      setTeamRosterError(
        "Impossible de charger la liste de l'équipe depuis la base (vérifiez que la table team_roster existe bien dans Supabase)."
      );
    }
  }
  ...
  async function handleAddTeamMember() {
    const resolved = capitalizeName(newTeamMemberName.trim());
    if (!resolved) {
      setTeamRosterError("Indiquez un prénom.");
      return;
    }
    if (teamRosterNames.some((n) => normalizeName(n) === normalizeName(resolved))) {
      setTeamRosterError("Cette personne fait déjà partie de l'équipe.");
      return;
    }
    setTeamRosterError("");
    try {
      await addTeamMember(resolved);
      await refreshTeamRoster();
      setNewTeamMemberName("");
    } catch (e) {
      console.error("Erreur d'ajout à l'équipe:", e);
      setTeamRosterError("Impossible d'ajouter cette personne.");
    }
  }

  async function handleRemoveTeamMember(prenom) {
    try {
      await removeTeamMember(prenom);
      await refreshTeamRoster();
      await refreshPlanning();
    } catch (e) {
      console.error("Erreur de suppression de l'équipe:", e);
      setTeamRosterError("Impossible de retirer cette personne.");
    }
  }

  async function handleToggleFreelance(prenom, isFreelance) {
    setTeamRoster((prev) => prev.map((r) => (r.prenom === prenom ? { ...r, isFreelance } : r)));
    try {
      await setTeamMemberFreelance(prenom, isFreelance);
    } catch (e) {
      console.error("Erreur de mise à jour freelance:", e);
      setTeamRosterError("Impossible de mettre à jour ce statut.");
      await refreshTeamRoster();
    }
  }

  async function refreshFreelanceAbsences() {
    try {
      const data = await loadFreelanceAbsences();
      setFreelanceAbsences(data);
    } catch (e) {
      console.error("Erreur de chargement:", e);
      setFreelanceError("Impossible de charger le répertoire des absences.");
    }
  }

  useEffect(() => {
    refreshFreelanceAbsences();
  }, []);

  async function handleAddFreelanceAbsence() {
    const missing = {};
    if (!freelancePrenom.trim()) missing.prenom = true;
    if (!freelanceDate) missing.date = true;
    if (!freelanceHeureDebut) missing.heureDebut = true;
    if (!freelanceHeureFin) missing.heureFin = true;

    if (Object.keys(missing).length > 0) {
      setFreelanceInvalidFields(missing);
      setFreelanceFormError("Certains champs ne sont pas complets.");
      return;
    }
    if (freelanceHeureFin <= freelanceHeureDebut) {
      setFreelanceInvalidFields({ heureDebut: true, heureFin: true });
      setFreelanceFormError("L'heure de fin doit être après l'heure de début.");
      return;
    }
    const resolvedFreelancePrenom = resolveKnownName(freelancePrenom);
    if (!freelanceNamesSet.has(resolvedFreelancePrenom)) {
      setFreelanceInvalidFields({ prenom: true });
      setFreelanceFormError("Ce prénom ne correspond à aucun membre en contrat freelance.");
      return;
    }

    setFreelanceFormError("");
    setFreelanceInvalidFields({});
    setFreelanceSaving(true);
    try {
      await addFreelanceAbsence({
        prenom: resolvedFreelancePrenom,
        date: freelanceDate,
        heureDebut: freelanceHeureDebut,
        heureFin: freelanceHeureFin,
      });
      await refreshFreelanceAbsences();
      setFreelanceError("");
    } catch (e) {
      console.error("Erreur d'ajout:", e);
      setFreelanceError("Impossible d'enregistrer cette absence.");
    } finally {
      setFreelanceSaving(false);
    }
    setFreelancePrenom("");
    setFreelanceDate("");
    setFreelanceHeureDebut("");
    setFreelanceHeureFin("");
  }

  async function handleDeleteFreelanceAbsence(id) {
    try {
      await deleteFreelanceAbsence(id);
      await refreshFreelanceAbsences();
    } catch (e) {
      console.error("Erreur de suppression:", e);
      setFreelanceError("Impossible de supprimer cette absence.");
    }
  }
  ...
  async function confirmVolunteer(id) {
    ...
    const parentEntry = (entries || []).find((e) => e.id === id);
    try {
      await addVolunteer(id, {
        prenom: resolveKnownName(nom),
        heureDebut: hd,
        heureFin: hf,
  ...
  const archivedFilteredSorted = useMemo(
    () =>
      archivedSorted.filter((e) => {
        const [y, m] = e.date.split("-");
        return (
          Number(y) === archiveYearFilter && (archiveMonthFilter === "toutes" || Number(m) === archiveMonthFilter)
        );
      }),
    [archivedSorted, archiveYearFilter, archiveMonthFilter]
  );

  // Répertoire des absences (Freelances) : actives (à venir) vs archivées (passées), avec
  // le même principe que les absences classiques.
  const freelanceSorted = useMemo(
    () => [...(freelanceAbsences || [])].sort((a, b) => (a.date + a.heureDebut).localeCompare(b.date + b.heureDebut)),
    [freelanceAbsences]
  );
  const freelanceFilteredSorted = useMemo(
    () =>
      freelanceSorted.filter((a) => {
        const [y, m] = a.date.split("-");
        return (
          Number(y) === freelanceYearFilter &&
          (freelanceMonthFilter === "toutes" || Number(m) === freelanceMonthFilter)
        );
      }),
    [freelanceSorted, freelanceYearFilter, freelanceMonthFilter]
  );
  ...
  const filteredSorted = useMemo(() => {
    const byStatus =
      statusFilter === "toutes"
        ? activeSorted
        : statusFilter === "non_complet"
        ? activeSorted.filter((e) => getEntryStatus(e) !== "complet")
        : activeSorted.filter((e) => getEntryStatus(e) === statusFilter);
    return byStatus.filter((e) => {
      const [y, m] = e.date.split("-");
      return Number(y) === mainYearFilter && (mainMonthFilter === "toutes" || Number(m) === mainMonthFilter);
    });
  }, [activeSorted, statusFilter, mainYearFilter, mainMonthFilter]);
  ...
  const sortedExchanges = useMemo(() => {
    if (!exchanges) return [];
    return [...exchanges].sort((a, b) => (a.date + a.heureDebut).localeCompare(b.date + b.heureDebut));
  }, [exchanges]);
  ...
  const archivedExchanges = useMemo(
    () =>
      sortedExchanges
        .filter((ex) => isExchangePast(ex))
        .sort((a, b) => (b.date + b.heureDebut).localeCompare(a.date + a.heureDebut)),
    [sortedExchanges]
  );
  const archivedExchangesFiltered = useMemo(
    () =>
      archivedExchanges.filter((ex) => {
        const [y, m] = ex.date.split("-");
        return (
          Number(y) === archiveExchangesYearFilter &&
          (archiveExchangesMonthFilter === "toutes" || Number(m) === archiveExchangesMonthFilter)
        );
      }),
    [archivedExchanges, archiveExchangesYearFilter, archiveExchangesMonthFilter]
  );
  ...
  // Heures d'absence par personne et par mois, pour le "Répertoire des absences (Freelances)"
  // — distinct de la table "absences" classique, mais décompté de la même façon.
  const freelanceAbsenceMinutesByPersonMonth = useMemo(() => {
    const map = new Map();
    for (const a of freelanceAbsences || []) {
      const monthKey = a.date.slice(0, 7);
      const minutes = timeToMinutes(a.heureFin) - timeToMinutes(a.heureDebut);
      if (minutes <= 0) continue;
      if (!map.has(a.prenom)) map.set(a.prenom, new Map());
      const monthMap = map.get(a.prenom);
      monthMap.set(monthKey, (monthMap.get(monthKey) || 0) + minutes);
    }
    return map;
  }, [freelanceAbsences]);
  ...
  const extraMinutesBreakdownByPersonMonth = useMemo(() => {
    ...
    for (const e of sorted) {
      ...
      const sundayNightMinutes = overlapMinutes(start, end, NIGHT_START, DAY_END); // dimanche : 21h-minuit seulement
      const dayMinutes = overlapMinutes(start, end, 0, NIGHT_START); // dimanche jour : minuit-21h
      ...
      for (const v of approved) {
        ...
        if (isHoliday) {
          ...
          current.sundayNight += sundayNightMinutes;
        } else {
  ...
  const adminStats = useMemo(() => {
    ...
    const pendingCount = activeSorted
  ...
  }, [activeSorted, activeExchanges, extraMinutesByPersonMonth, statsYearFilter, statsMonthFilter]);
  ...
  const knownNamesNormalized = useMemo(
    () => new Set([...teamMembers, ...teamRosterNames, "Ghost"].map(normalizeName)),
    [teamMembers, teamRosterNames]
  );
  ...
  function resolveKnownName(raw) {
    ...
    for (const known of [...teamMembers, ...teamRosterNames, "Ghost"]) {
  ...
  const weeklyHoursByPerson = useMemo(() => {
    ...
    const HOLIDAY_EXCLUDED_NAMES = new Set(["Clémence", "Sylvain", "Capucine"]);
    ...
    const allNames = [...new Set([...teamMembers, ...teamRosterNames])];
    return allNames.map((prenom) => ({
      prenom,
      ...
      absenceMinutes:
        sumExtra(absenceMinutesByPersonMonth.get(prenom)) + sumExtra(freelanceAbsenceMinutesByPersonMonth.get(prenom)),
    }));
  }, [
    teamMembers,
    teamRosterNames,
    ...
    extraMinutesByPersonMonth,
    absenceMinutesByPersonMonth,
    freelanceAbsenceMinutesByPersonMonth,
  ]);

  // Heures contractuelles majorées : portions du planning RÉGULIER (pas des heures
  // supplémentaires) qui tombent sur un créneau majoré — soir de semaine, dimanche, ou jour
  // férié. Clémence est exclue (ses soirées du mardi/mercredi sont des heures supplémentaires
  // récurrentes, déjà traitées ailleurs, pas des heures contractuelles majorées).
  const contractualMajoreesByPerson = useMemo(() => {
    const monthsToProcess =
      majoreesMonthFilter === "toutes" ? Array.from({ length: 12 }, (_, i) => i + 1) : [majoreesMonthFilter];
    const byPerson = new Map(); // prenom -> { weekdayEvening, sundayDay, sundayNight, holiday }

    const addTo = (prenom, key, minutes) => {
      if (minutes <= 0) return;
      if (!byPerson.has(prenom)) byPerson.set(prenom, { weekdayEvening: 0, sundayDay: 0, sundayNight: 0, holiday: 0 });
      byPerson.get(prenom)[key] += minutes;
    };

    for (const m of monthsToProcess) {
      const daysInMonth = new Date(majoreesYearFilter, m, 0).getDate();
      const holidaysThisYear = getFrenchHolidays(majoreesYearFilter);
      for (let day = 1; day <= daysInMonth; day++) {
        const dateObj = new Date(majoreesYearFilter, m - 1, day);
        const dateStr = `${majoreesYearFilter}-${String(m).padStart(2, "0")}-${String(day).padStart(2, "0")}`;
        const isHoliday = holidaysThisYear.has(dateStr);
        const jsDay = dateObj.getDay(); // 0 = dimanche
        const jour = JOURS[(jsDay + 6) % 7];
        const dayShifts = (planningByDay.find((d) => d.jour === jour) || {}).shifts || [];
        ...
        const allShiftsToday = [
          ...dayShifts.filter((s) => !OVERRIDDEN_NAMES.has(s.prenom)),
          ...Object.entries(TICKETING_HOURS_OVERRIDE).flatMap(([prenom, overrideShifts]) =>
            overrideShifts.filter((os) => os.jour === jour).map((os) => ({ ...os, prenom }))
          ),
        ];

        for (const s of allShiftsToday) {
          if (s.prenom === "Clémence") continue; // exclue : heures supplémentaires récurrentes, pas majorées
          const start = timeToMinutes(s.heureDebut);
          const end = effectiveEndMinutes(s.heureFin);
          const nightMinutes = overlapMinutes(start, end, 21 * 60, 24 * 60) + overlapMinutes(start, end, 0, 6 * 60);
          const sundayNightMinutes = overlapMinutes(start, end, 21 * 60, 24 * 60); // dimanche : 21h-minuit seulement
          const dayMinutes = overlapMinutes(start, end, 0, 21 * 60); // dimanche jour : minuit-21h

          if (isHoliday) {
            addTo(s.prenom, "holiday", end - start);
          } else if (jsDay === 0) {
            // Dimanche : 6h-21h (25%) et 21h-minuit (50%)
            addTo(s.prenom, "sundayDay", dayMinutes);
            addTo(s.prenom, "sundayNight", sundayNightMinutes);
          } else {
            // Lundi à samedi : 21h-6h, 25%
            addTo(s.prenom, "weekdayEvening", nightMinutes);
          }
        }
      }
    }

    return [...byPerson.entries()].map(([prenom, d]) => ({
      prenom,
      total: d.weekdayEvening + d.sundayDay + d.sundayNight + d.holiday,
      ...d,
    }));
  }, [planningByDay, majoreesYearFilter, majoreesMonthFilter]);
  ...
  function exportTableToPDF(title, headers, rows, onBlocked, period) {
    const win = window.open("", "_blank");
    if (!win) {
    ...
    win.document.write(`<!DOCTYPE html>
      <html>
        ...
        <body style="font-family: Arial, Helvetica, sans-serif; padding: 24px; color: #0f172a;">
          ...
          ${period ? `<p style="font-size:13px;color:#475569;margin-bottom:4px;">Période : ${period}</p>` : ""}
          <p style="font-size:12px;color:#94a3b8;margin-bottom:16px;">Exporté le ${new Date().toLocaleDateString(
            "fr-FR"
          )}</p>
  ...
  function handleExportContractualMajoreesPDF() {
    const rows = contractualMajoreesByPerson
      .filter((row) => row.total > 0 && !freelanceNamesSet.has(row.prenom))
      .sort((a, b) => a.prenom.localeCompare(b.prenom))
      .map((row) => [
        row.prenom,
        row.total > 0 ? formatDurationRounded(row.total) : "—",
        row.weekdayEvening > 0 ? formatDurationRounded(row.weekdayEvening) : "—",
        row.sundayDay > 0 ? formatDurationRounded(row.sundayDay) : "—",
        row.sundayNight > 0 ? formatDurationRounded(row.sundayNight) : "—",
        row.holiday > 0 ? formatDurationRounded(row.holiday) : "—",
      ]);
    const period =
      majoreesMonthFilter === "toutes"
        ? `Tous les mois ${majoreesYearFilter}`
        : `${MONTH_NAMES[majoreesMonthFilter - 1]} ${majoreesYearFilter}`;
    exportTableToPDF(
      "Heures contractuelles majorées",
      ["Personne", "Total majoré", "Lun-Sam nuit (25%)", "Dim jour (25%)", "Dim nuit (50%)", "Jour férié (50%)"],
      rows,
  ...
  function handleExportWeeklyHoursPDF() {
    const rows = weeklyHoursByPerson.map((row) => [
      row.prenom,
      row.monthlyMinutes > 0 ? formatDurationRounded(row.monthlyMinutes) : "—",
      row.extraMinutes > 0 ? formatDurationRounded(row.extraMinutes) : "—",
      row.absenceMinutes > 0 ? formatDurationRounded(row.absenceMinutes) : "—",
      formatDurationRounded(row.monthlyMinutes + row.extraMinutes - row.absenceMinutes),
    ]);
    const period =
      hoursMonthFilter === "toutes"
        ? `Tous les mois ${hoursYearFilter}`
        : `${MONTH_NAMES[hoursMonthFilter - 1]} ${hoursYearFilter}`;
    exportTableToPDF(
  ...
  function handleExportTicketsPDF() {
    ...
    const period =
      monthFilter === "toutes" ? `Tous les mois ${yearFilter}` : `${MONTH_NAMES[monthFilter - 1]} ${yearFilter}`;
    exportTableToPDF(
      "Tickets restaurant",
  ...
  function handleExportArchivedAbsencesPDF() {
    const rows = archivedFilteredSorted.map((e) => {
      const approved = e.volunteers.filter((v) => v.status === "approved" || !v.status);
      const remplacants = approved.map((v) => `${v.prenom} (${v.heureDebut}–${v.heureFin})`).join(", ") || "—";
      return [e.prenom, formatDateLong(e.date), `${e.heureDebut} – ${e.heureFin}`, remplacants];
    });
    const period =
      archiveMonthFilter === "toutes"
        ? `Tous les mois ${archiveYearFilter}`
        : `${MONTH_NAMES[archiveMonthFilter - 1]} ${archiveYearFilter}`;
    exportTableToPDF(
      "Absences archivées",
      ["Personne", "Date", "Créneau", "Remplacé par"],
      rows,
      () => setError("La fenêtre d'export a été bloquée par le navigateur. Autorisez les pop-ups pour ce site puis réessayez."),
      period
    );
  ...
  function handleExportArchivedExchangesPDF() {
    const rows = archivedExchangesFiltered.map((ex) => [
      ex.prenom,
      `${formatDateLong(ex.date)} · ${ex.heureDebut}–${ex.heureFin}`,
      ex.collegue || "—",
      ex.collegue ? `${formatDateLong(ex.collegueDate)} · ${ex.collegueHeureDebut}–${ex.collegueHeureFin}` : "—",
    ]);
    const period =
      archiveExchangesMonthFilter === "toutes"
        ? `Tous les mois ${archiveExchangesYearFilter}`
        : `${MONTH_NAMES[archiveExchangesMonthFilter - 1]} ${archiveExchangesYearFilter}`;
    exportTableToPDF(
      "Échanges archivés",
      ["Personne", "Créneau initial", "Collègue", "Créneau échangé"],
      rows,
      () => setError("La fenêtre d'export a été bloquée par le navigateur. Autorisez les pop-ups pour ce site puis réessayez."),
      period
    );
  ...
  const ticketsRestaurantByPerson = useMemo(() => {
    ...
    for (const m of monthsToProcess) {
      ...
      for (let day = 1; day <= daysInMonth; day++) {
        ...
        const HOLIDAY_EXCLUDED_TICKET_NAMES = new Set(["Clémence", "Sylvain", "Capucine"]);
    ...
    return [...byPerson.entries()]
      ...
      .filter((r) => !freelanceNamesSet.has(r.prenom) && !TICKETS_EXCLUDED_NAMES.has(r.prenom))
      .sort((a, b) => {
  ...
  }, [planningByDay, sorted, yearFilter, monthFilter, selectedMonthKey, freelanceNamesSet]);
        ...
        <nav className="flex flex-col gap-1">
          {[
            ...
            [
              ...
              { key: "freelance", label: "Répertoire des absences (Freelances)", icon: ListTodo },
              { key: "echanges", label: "Échanges", icon: Repeat },
            ],
            [{ key: "archive", label: "Archives", icon: Archive }],
          ].map((group, groupIndex) => {
            const visibleItems = group.filter((item) => item.key !== "admin" || isAdmin);
            if (visibleItems.length === 0) return null;
            return (
              <div
                key={groupIndex}
                className={groupIndex > 0 ? "pt-3 mt-2 border-t border-slate-100" : ""}
              >
                {visibleItems.map((item) => (
                  <button
                    key={item.key}
                    onClick={() => setPage(item.key)}
                    className={`w-full flex items-center gap-2.5 text-sm font-medium px-3 py-2.5 rounded-lg transition-colors text-left ${
            ...
            {
              {
                ...
                freelance: "Répertoire des absences (Freelances)",
                echanges: "Échanges",
                archive: "Archives",
              }[page]
            }
          ...
          {[
            ...
            return (
              <div key={groupIndex} className="flex items-center gap-1.5">
                {groupIndex > 0 && <span className="w-px h-5 bg-slate-200 mx-1" />}
                {visibleItems.map((item) => (
                  <button
                    key={item.key}
                    onClick={() => setPage(item.key)}
        ...
        {page === "admin" ? (
          <div className="space-y-6">
            <section className="bg-white border border-slate-200 rounded-2xl shadow-sm p-5 sm:p-6">
              ...
              <div className="grid grid-cols-4 gap-4">
                ...
                >
                  <p
                    className={`text-2xl font-bold ${
                      adminStats.pendingCount > 0 ? "text-orange-800" : "text-slate-900"
                    }`}
                  >
                ...
                <div
                  className={`rounded-xl border p-4 ${
                    adminStats.pendingExchangesCount > 0
                      ? "border-orange-300 bg-orange-100"
                      : "border-slate-200"
                  }`}
                >
                  <p
                    className={`text-2xl font-bold ${
                      adminStats.pendingExchangesCount > 0 ? "text-orange-800" : "text-slate-900"
                    }`}
                  >
                    {adminStats.pendingExchangesCount}
                  </p>
                  <p className="text-xs text-slate-500 mt-1">
            ...
            <section className="bg-white border border-slate-200 rounded-2xl shadow-sm p-5 sm:p-6">
                  ...
                  {showHoursInfo && (
                    <div className="absolute top-6 left-0 z-10 w-80 bg-white border border-slate-200 rounded-xl shadow-lg p-4 normal-case">
                      ...
                      <p className="text-xs text-slate-500 leading-relaxed mb-4">
                        La mention <span className="font-medium">(Free)</span> à côté d'un prénom indique
                        une personne en contrat freelance.
                      </p>
              ...
              {teamRosterError && <p className="text-xs text-rose-600 mb-3">{teamRosterError}</p>}
                  ...
                  <table className="w-full text-sm text-center">
                    <thead>
                      <tr className="text-center text-[11px] font-semibold text-slate-400 uppercase tracking-wide">
                        <th className="pb-2 pr-3 text-left">Personne</th>
                        <th className="pb-2 pr-3">Freelance</th>
                    ...
                    <tbody>
                      {weeklyHoursByPerson
                        .filter((row) => teamRosterNames.includes(row.prenom) && row.prenom !== "Capucine")
                        .sort((a, b) => {
                          const order = teamRosterNames.filter((n) => n !== "Capucine");
                        ...
                        return (
                          <tr key={row.prenom} className="border-t border-slate-100">
                            <td className="py-2 pr-3 font-medium text-slate-900 text-left">
                              {displayPersonName(row.prenom)}
                            </td>
                            <td className="py-2 pr-3">
                              <input
                                type="checkbox"
                                checked={freelanceNamesSet.has(row.prenom)}
                                onChange={(ev) => handleToggleFreelance(row.prenom, ev.target.checked)}
                                disabled={!isAdmin}
                                className="w-4 h-4 rounded border-slate-300 text-violet-600 focus:ring-violet-500"
                              />
                            </td>
                            ...
                            <td className="py-2 pr-3 font-medium text-slate-900">
                              {formatDurationRounded(totalMinutes)}
                            </td>
                            <td className="py-2 pr-3">
                              <input
                                type="number"
                            ...
                            <td className="py-2">
                              <button
                                onClick={() => handleRemoveTeamMember(row.prenom)}
              ...
              <div className="mt-3 flex items-center gap-1.5">
                <PrenomAutocomplete
                  ...
                  onChange={setNewTeamMemberName}
                  options={teamRosterNames}
                  className="w-40 px-2.5 py-1.5 rounded-lg border border-slate-200 focus:outline-none focus:ring-2 focus:ring-violet-500 text-xs"
                />
                <button
                  onClick={handleAddTeamMember}
                  className="inline-flex items-center gap-1 text-xs font-medium px-2.5 py-1.5 rounded-lg bg-violet-600 hover:bg-violet-700 text-white transition-colors"
                >
                  <Plus className="w-3.5 h-3.5" /> Ajouter une personne dans l'équipe
                </button>
              </div>

              {(() => {
                const rows = ["Capucine"].map((prenom) => {
                          ...
                          {rows.map((row) => {
                            ...
                            return (
                              <tr key={prenom} className="border-t border-slate-100">
                                <td className="py-2 pr-3 font-medium text-slate-900 text-left">
                                  {displayPersonName(prenom)}
                                </td>
                                <td className="py-2 pr-3">
                                  <input
                                    type="checkbox"
                                    checked={freelanceNamesSet.has(prenom)}
                                    onChange={(ev) => handleToggleFreelance(prenom, ev.target.checked)}
                                    disabled={!isAdmin}
                                    className="w-4 h-4 rounded border-slate-300 text-violet-600 focus:ring-violet-500"
                                  />
                                </td>
                                ...
                                <td className="py-2">
                                  <button
                                    onClick={() => handleRemoveTeamMember(prenom)}
                                    title="Retirer de l'équipe"
                                    className="p-1.5 rounded-md text-slate-300 hover:text-rose-600 hover:bg-rose-50 transition-colors"
                                  >
                                    <Trash2 className="w-3.5 h-3.5" />
            ...
            <section className="bg-white border border-slate-200 rounded-2xl shadow-sm p-5 sm:p-6">
              <div className="flex items-center justify-between flex-wrap gap-2 mb-3">
                <p className="flex items-center gap-1.5 text-sm font-semibold text-slate-600 uppercase tracking-wide">
                  <Clock className="w-4 h-4" /> Heures contractuelles majorées
                </p>
                <div className="flex items-center gap-2">
                  <select
                    value={majoreesMonthFilter}
                    onChange={(ev) => {
                      const v = ev.target.value;
                      setMajoreesMonthFilter(v === "toutes" ? "toutes" : Number(v));
                    }}
                    className="text-xs font-medium px-2.5 py-1.5 rounded-lg border border-slate-200 text-slate-600 focus:outline-none focus:ring-2 focus:ring-violet-500"
                  >
                    <option value="toutes">Tous les mois</option>
                  ...
                  <select
                    value={majoreesYearFilter}
                    onChange={(ev) => setMajoreesYearFilter(Number(ev.target.value))}
                    className="text-xs font-medium px-2.5 py-1.5 rounded-lg border border-slate-200 text-slate-600 focus:outline-none focus:ring-2 focus:ring-violet-500"
                  >
                    {availableYears.map((y) => (
                      <option key={y} value={y}>
                  ...
                  <button
                    onClick={handleExportContractualMajoreesPDF}
                    className="inline-flex items-center gap-1.5 text-xs font-medium px-2.5 py-1.5 rounded-lg border border-slate-200 text-slate-600 hover:border-violet-300 hover:text-violet-700 transition-colors"
                  >
                    <Save className="w-3.5 h-3.5" /> Exporter en PDF
                  </button>
              ...
              {contractualMajoreesByPerson.filter((row) => row.total > 0 && !freelanceNamesSet.has(row.prenom))
                .length === 0 ? (
                <p className="text-sm text-slate-400">Aucune heure majorée pour cette période.</p>
              ) : (
                <div className="overflow-x-auto">
                  <table className="w-full text-sm text-center">
                    <thead>
                      ...
                      <tr className="text-center text-[11px] font-semibold text-slate-400 uppercase tracking-wide">
                        <th className="pb-2 pr-3 text-left">Personne</th>
                        <th className="pb-2 pr-3">Total majoré</th>
                        <th className="pb-2 pr-3 pl-3 border-l border-slate-200">LUN-SAM NUIT (21h-6h) · 25%</th>
                        <th className="pb-2 pr-3">DIM JOUR (minuit-21h) · 25%</th>
                        <th className="pb-2 pr-3">DIM NUIT (21h-minuit) · 50%</th>
                        <th className="pb-2">JOUR FÉRIÉ · 50%</th>
                      </tr>
                    </thead>
                    <tbody>
                      {contractualMajoreesByPerson
                        .filter((row) => row.total > 0 && !freelanceNamesSet.has(row.prenom))
                        .sort((a, b) => a.prenom.localeCompare(b.prenom))
                        .map((row) => (
                        <tr key={row.prenom} className="border-t border-slate-100">
                  ...
                  <table className="w-full text-sm text-center">
                    <thead>
                      ...
                      <tr className="text-center text-[11px] font-semibold text-slate-400 uppercase tracking-wide">
                        ...
                        <th className="pb-2 pr-3">DIM JOUR (minuit-21h)</th>
                        <th className="pb-2 pr-3">DIM NUIT (21h-minuit)</th>
                    ...
                    <tbody>
                      {personSummaryTable.map((row) => (
                        <tr key={row.prenom} className="border-t border-slate-100">
                          ...
                          <td className="py-2 pr-3 pl-3 text-slate-600 border-l border-slate-200">
                            {row.nightWeekdayMinutes > 0 ? formatDurationRounded(row.nightWeekdayMinutes) : "—"}
                          </td>
                          <td className="py-2 pr-3 text-slate-600">
                            {row.sundayDayMinutes > 0 ? formatDurationRounded(row.sundayDayMinutes) : "—"}
                          </td>
                          <td className="py-2 pr-3 text-slate-600">
                            {row.sundayNightMinutes > 0 ? formatDurationRounded(row.sundayNightMinutes) : "—"}
                          </td>
                          <td className="py-2 text-slate-600">
                            {row.holidayMinutes > 0 ? formatDurationRounded(row.holidayMinutes) : "—"}
                          </td>
                        </tr>
                      ))}
                    </tbody>
            ...
            {isAdmin && (
              <section className="bg-white border border-slate-200 rounded-2xl shadow-sm p-5 sm:p-6 mb-8">
                <p className="flex items-center gap-1.5 text-sm font-semibold text-slate-600 uppercase tracking-wide mb-4">
                  <Users className="w-3.5 h-3.5" /> Renseigner un créneau
                </p>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-6 gap-3 sm:gap-4">
                  <div className="col-span-2">
                    ...
                    <PrenomAutocomplete
                      value={planPrenom}
                      onChange={(v) => setPlanPrenom(capitalizeName(v))}
                      options={[...teamRosterNames, "Ghost"]}
                      invalid={planInvalidFields.prenom}
                    />
                  </div>
                  <div className="col-span-2">
                    ...
                    <select
                      value={planJour}
                      onChange={(ev) => setPlanJour(ev.target.value)}
                ...
                {showPlanningInfo && (
                  <div className="absolute top-6 left-0 z-10 w-80 bg-white border border-slate-200 rounded-xl shadow-lg p-4 normal-case">
                    ...
                    <p className="text-xs text-slate-500 leading-relaxed">
                      ...
                      <br />
                      Pour les créneaux affichés en deux couleurs, la partie{" "}
                      <span className="text-violet-600 font-medium">violette</span> correspond à la
                      portion du créneau sur laquelle la personne traite des tickets ; la partie grise
                      correspond au reste du créneau.
                    </p>
                  </div>
                )}
                </div>
                        ...
                        {people.length === 0 ? (
                          ...
                          <div className="border border-slate-100 rounded-lg overflow-hidden">
                            ...
                            {people.map(({ prenom, segments }) => (
                              ...
                              >
                                <span className="w-28 shrink-0 text-xs font-medium truncate text-slate-700">
                                  {displayPersonName(prenom)}
                                </span>
                                ...
                                <div className="relative flex-1 h-5 bg-slate-50 rounded">
                                  ...
                                  {segments.map((s) => {
                                    ...
                                    const chunks = splitSegmentByTicketZones(jour, prenom, start, end);
          ...
          <>
            <div className="bg-white border border-slate-200 rounded-2xl shadow-sm p-5 sm:p-6 mb-8">
              <p className="flex items-center gap-1.5 text-sm font-semibold text-slate-600 uppercase tracking-wide mb-4">
                <ListTodo className="w-4 h-4" /> Renseigner une absence
              </p>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-6 gap-3 sm:gap-4">
                <div className="col-span-2">
                  <label className="block text-sm font-medium text-slate-700 mb-1.5">Prénom</label>
                  <input
                    value={freelancePrenom}
                    onChange={(e) => setFreelancePrenom(capitalizeName(e.target.value))}
                    className={`w-full px-3 py-2.5 rounded-lg border focus:outline-none focus:ring-2 text-sm ${
                      freelanceInvalidFields.prenom
                        ? "border-rose-400 focus:ring-rose-500 focus:border-rose-500"
                        : "border-slate-300 focus:ring-violet-500 focus:border-violet-500"
                    }`}
                  />
                ...
                <div className="col-span-2">
                  ...
                  <input
                    type="date"
                    value={freelanceDate}
                    onChange={(e) => setFreelanceDate(e.target.value)}
                    className={`w-full px-3 py-2.5 rounded-lg border focus:outline-none focus:ring-2 text-sm ${
                      freelanceInvalidFields.date
                        ? "border-rose-400 focus:ring-rose-500 focus:border-rose-500"
                        : "border-slate-300 focus:ring-violet-500 focus:border-violet-500"
                    }`}
                  />
                ...
                <div className="col-span-1">
                  ...
                  <input
                    type="time"
                    value={freelanceHeureDebut}
                    onChange={(e) => setFreelanceHeureDebut(e.target.value)}
                    className={`w-full px-3 py-2.5 rounded-lg border focus:outline-none focus:ring-2 text-sm ${
                      freelanceInvalidFields.heureDebut
                        ? "border-rose-400 focus:ring-rose-500 focus:border-rose-500"
                        : "border-slate-300 focus:ring-violet-500 focus:border-violet-500"
                    }`}
                  />
                ...
                <div className="col-span-1">
                  ...
                  <input
                    type="time"
                    value={freelanceHeureFin}
                    onChange={(e) => setFreelanceHeureFin(e.target.value)}
                    className={`w-full px-3 py-2.5 rounded-lg border focus:outline-none focus:ring-2 text-sm ${
                      freelanceInvalidFields.heureFin
                        ? "border-rose-400 focus:ring-rose-500 focus:border-rose-500"
                        : "border-slate-300 focus:ring-violet-500 focus:border-violet-500"
                    }`}
                  />
              ...
              <div className="mt-5 flex justify-end">
                <button
                  type="button"
                  onClick={handleAddFreelanceAbsence}
                  disabled={freelanceSaving}
                ...
                >
                  {freelanceSaving ? <Loader2 className="w-4 h-4 animate-spin" /> : <Plus className="w-4 h-4" />}
                  Ajouter l'absence
                </button>
              </div>

              {freelanceError && <p className="mt-3 text-sm text-rose-600">{freelanceError}</p>}
            </div>

            <section className="bg-white border border-slate-200 rounded-2xl shadow-sm p-5 sm:p-6">
              <div className="flex items-center justify-between flex-wrap gap-2 mb-3">
                <p className="flex items-center gap-1.5 text-sm font-semibold text-slate-600 uppercase tracking-wide">
                  <ListTodo className="w-4 h-4" /> Absences renseignées{" "}
                  {freelanceAbsences ? `(${freelanceFilteredSorted.length})` : ""}
                </p>
                <div className="flex items-center gap-2">
                  <select
                    value={freelanceMonthFilter}
                    onChange={(ev) => {
                      const v = ev.target.value;
                      setFreelanceMonthFilter(v === "toutes" ? "toutes" : Number(v));
                    }}
                    className="text-xs font-medium px-2.5 py-1.5 rounded-lg border border-slate-200 text-slate-600 focus:outline-none focus:ring-2 focus:ring-violet-500"
                  >
                    <option value="toutes">Tous les mois</option>
                  ...
                  <select
                    value={freelanceYearFilter}
                    onChange={(ev) => setFreelanceYearFilter(Number(ev.target.value))}
                    className="text-xs font-medium px-2.5 py-1.5 rounded-lg border border-slate-200 text-slate-600 focus:outline-none focus:ring-2 focus:ring-violet-500"
                  >
                    {availableYears.map((y) => (
                      <option key={y} value={y}>
              ...
              {freelanceAbsences === null ? (
                <div className="flex items-center gap-2 text-slate-400 text-sm py-8 justify-center">
                  <Loader2 className="w-4 h-4 animate-spin" /> Chargement…
                </div>
              ) : freelanceFilteredSorted.length === 0 ? (
                ...
                <ul className="space-y-2">
                  {freelanceFilteredSorted.map((a) => (
                    ...
                    >
                      ...
                      <button
                        onClick={() => handleDeleteFreelanceAbsence(a.id)}
                        title="Supprimer"
                                  ...
                                  {ex.status !== "open" && (
                                    <div>
                                      <div className="flex items-center gap-1.5 text-slate-400 text-xs mb-1">
                                        <ArrowLeftRight className="w-3.5 h-3.5" /> Échange avec
                                      </div>
                                      ...
                                      <div className="mt-1 space-y-1">
                                        <p className="text-slate-600">
                                          ...
                                          {formatDateLong(ex.collegueDate)}
                                        </p>
                                        <p className="text-slate-600">
                                          <span className="text-[11px] font-semibold text-slate-400 uppercase tracking-wide mr-1.5">
                                            Créneau
                                          </span>
                                          {ex.collegueHeureDebut} – {ex.collegueHeureFin}
                                        </p>
                                      </div>
                                    </div>
                                  )}
          ...
          <section className="bg-white border border-slate-200 rounded-2xl shadow-sm p-5 sm:p-6 mb-8">
            <div className="flex items-center justify-between flex-wrap gap-2 mb-4">
              <p className="flex items-center gap-1.5 text-sm font-semibold text-slate-600 uppercase tracking-wide">
                <Archive className="w-4 h-4" /> Absences archivées{" "}
                {archivedSorted.length ? `(${archivedFilteredSorted.length})` : ""}
              </p>
              <div className="flex items-center gap-2">
                <select
                  ...
                  onChange={(ev) => {
                    const v = ev.target.value;
                    setArchiveMonthFilter(v === "toutes" ? "toutes" : Number(v));
                    setArchivedAbsencesVisibleCount(10);
                  }}
                  className="text-xs font-medium px-2.5 py-1.5 rounded-lg border border-slate-200 text-slate-600 focus:outline-none focus:ring-2 focus:ring-violet-500"
                >
                  <option value="toutes">Tous les mois</option>
                ...
                <select
                  ...
                  onChange={(ev) => {
                    setArchiveYearFilter(Number(ev.target.value));
                    setArchivedAbsencesVisibleCount(10);
                  }}
                  className="text-xs font-medium px-2.5 py-1.5 rounded-lg border border-slate-200 text-slate-600 focus:outline-none focus:ring-2 focus:ring-violet-500"
                >
                ...
                <button
                  onClick={handleExportArchivedAbsencesPDF}
                  className="inline-flex items-center gap-1.5 text-xs font-medium px-2.5 py-1.5 rounded-lg border border-slate-200 text-slate-600 hover:border-violet-300 hover:text-violet-700 transition-colors"
                >
                  <Save className="w-3.5 h-3.5" /> Exporter en PDF
                </button>
            ...
            {archivedFilteredSorted.length === 0 ? (
              <div className="text-center py-10 text-slate-400 text-sm">
                Aucune absence archivée pour cette période.
              </div>
            ) : (
              <ul className="space-y-2">
                {archivedFilteredSorted.slice(0, archivedAbsencesVisibleCount).map((e) => {
                  const approved = e.volunteers.filter((v) => v.status === "approved" || !v.status);
                        ...
                        {approved.length > 0 && (
                          <div className="flex flex-wrap gap-1.5 mt-2">
                            {approved.map((v, i) => (
                              <span
                                key={i}
                                className="text-xs font-medium bg-slate-100 text-slate-700 px-2 py-1 rounded-lg"
                              >
                                <span className="font-semibold text-slate-900">{v.prenom}</span>{" "}
            ...
            {archivedFilteredSorted.length > archivedAbsencesVisibleCount && (
              <div className="mt-4 flex justify-center">
                <button
                  onClick={() => setArchivedAbsencesVisibleCount((n) => n + 10)}
                  className="text-sm font-medium px-4 py-2 rounded-lg border border-slate-200 text-slate-600 hover:border-violet-300 hover:text-violet-700 transition-colors"
                >
                  Charger plus
                </button>
              </div>
          ...
          <section className="bg-white border border-slate-200 rounded-2xl shadow-sm p-5 sm:p-6">
            <div className="flex items-center justify-between flex-wrap gap-2 mb-4">
              <p className="flex items-center gap-1.5 text-sm font-semibold text-slate-600 uppercase tracking-wide">
                <ArrowLeftRight className="w-4 h-4" /> Échanges archivés{" "}
                {archivedExchangesFiltered.length ? `(${archivedExchangesFiltered.length})` : ""}
              </p>
              <div className="flex items-center gap-2">
                <select
                  value={archiveExchangesMonthFilter}
                  onChange={(ev) => {
                    const v = ev.target.value;
                    setArchiveExchangesMonthFilter(v === "toutes" ? "toutes" : Number(v));
                    setArchivedExchangesVisibleCount(10);
                  }}
                  className="text-xs font-medium px-2.5 py-1.5 rounded-lg border border-slate-200 text-slate-600 focus:outline-none focus:ring-2 focus:ring-violet-500"
                >
                  <option value="toutes">Tous les mois</option>
                ...
                <select
                  value={archiveExchangesYearFilter}
                  onChange={(ev) => {
                    setArchiveExchangesYearFilter(Number(ev.target.value));
                    setArchivedExchangesVisibleCount(10);
                  }}
                  className="text-xs font-medium px-2.5 py-1.5 rounded-lg border border-slate-200 text-slate-600 focus:outline-none focus:ring-2 focus:ring-violet-500"
                >
                ...
                <button
                  onClick={handleExportArchivedExchangesPDF}
                  className="inline-flex items-center gap-1.5 text-xs font-medium px-2.5 py-1.5 rounded-lg border border-slate-200 text-slate-600 hover:border-violet-300 hover:text-violet-700 transition-colors"
                >
                  <Save className="w-3.5 h-3.5" /> Exporter en PDF
                </button>
            ...
            {archivedExchangesFiltered.length === 0 ? (
              <div className="text-center py-10 text-slate-400 text-sm">
                Aucun échange archivé pour cette période.
              </div>
            ) : (
              <ul className="space-y-2">
                {archivedExchangesFiltered.slice(0, archivedExchangesVisibleCount).map((ex) => (
                  <li
                    key={ex.id}
                    className="border border-slate-100 rounded-xl p-3 flex items-start justify-between gap-3 opacity-80"
                  >
            ...
            {archivedExchangesFiltered.length > archivedExchangesVisibleCount && (
              <div className="mt-4 flex justify-center">
                <button
                  onClick={() => setArchivedExchangesVisibleCount((n) => n + 10)}
                  className="text-sm font-medium px-4 py-2 rounded-lg border border-slate-200 text-slate-600 hover:border-violet-300 hover:text-violet-700 transition-colors"
                >
                  Charger plus
                </button>
        ...
        >
          ...
          <div className="grid grid-cols-2 sm:grid-cols-6 gap-3 sm:gap-4">
            <div className="col-span-2">
              ...
              <PrenomAutocomplete
                value={prenom}
                onChange={(v) => setPrenom(capitalizeName(v))}
                options={[...teamRosterNames, "Ghost"]}
                invalid={invalidFields.prenom}
              />
            </div>
            <div className="col-span-2">
              <label className="block text-sm font-medium text-slate-700 mb-1.5">Date</label>
        ...
        <section className="bg-white border border-slate-200 rounded-2xl shadow-sm p-5 sm:p-6">
          <div className="flex items-center justify-between flex-wrap gap-3 mb-3">
            <h2 className="flex items-center gap-1.5 text-sm font-semibold text-slate-500 uppercase tracking-wide">
              <Calendar className="w-4 h-4" /> Absences renseignées {entries ? `(${filteredSorted.length})` : ""}
            </h2>

            <div className="flex items-center gap-2">
              <select
                value={mainMonthFilter}
                onChange={(ev) => {
                  const v = ev.target.value;
                  setMainMonthFilter(v === "toutes" ? "toutes" : Number(v));
                }}
                className="text-xs font-medium px-2.5 py-1.5 rounded-lg border border-slate-200 text-slate-600 focus:outline-none focus:ring-2 focus:ring-violet-500"
              >
                <option value="toutes">Tous les mois</option>
              ...
              <select
                ...
                onChange={(ev) => setMainYearFilter(Number(ev.target.value))}
                className="text-xs font-medium px-2.5 py-1.5 rounded-lg border border-slate-200 text-slate-600 focus:outline-none focus:ring-2 focus:ring-violet-500"
              >
                {availableYears.map((y) => (
                  <option key={y} value={y}>
                                ...
                                {isAdmin && (
                                  <div className="flex items-center gap-1 shrink-0 mr-4">
                                    <button
                                      ...
                                      title="Valider"
                                      className="p-1 rounded-md text-emerald-700 hover:text-white hover:bg-emerald-600 border border-emerald-300 transition-colors"
                                    >
                                    ...
                                    <button
                                      ...
                                      title="Refuser"
                                      className="p-1 rounded-md text-rose-700 hover:text-white hover:bg-rose-600 border border-rose-300 transition-colors"
                                    >
                                      <X className="w-3.5 h-3.5" />
                                    </button>
                                  </div>
