...
export default function AbsenceTracker() {
      ...
      for (let day = 1; day <= daysInMonth; day++) {
        ...
        for (const s of allShiftsToday) {
          ...
          if (isHoliday) {
            ...
            continue; // priorité au jour férié, jamais compté deux fois dans une autre colonne
          }

          if (jsDay >= 1 && jsDay <= 6) {
            // Lundi à samedi : 21h-minuit, 25%
            addTo(s.prenom, "weekdayEvening", overlapMinutes(start, end, 21 * 60, 24 * 60));
          } else if (jsDay === 0) {
            // Dimanche : minuit-21h (25%) et 21h-minuit (50%)
            addTo(s.prenom, "sundayDay", overlapMinutes(start, end, 0, 21 * 60));
            addTo(s.prenom, "sundayNight", overlapMinutes(start, end, 21 * 60, 24 * 60));
          }
        }
      }
    }
  ...
  function handleExportContractualMajoreesPDF() {
    ...
    exportTableToPDF(
      ...
      ["Personne", "Total majoré", "Lun-Sam soir (25%)", "Dim jour (25%)", "Dim soir (50%)", "Jour férié (50%)"],
      rows,
                  ...
                  <table className="w-full text-sm text-center">
                    <thead>
                      ...
                      <tr className="text-center text-[11px] font-semibold text-slate-400 uppercase tracking-wide">
                        ...
                        <th className="pb-2 pr-3 pl-3 border-l border-slate-200">LUN-SAM SOIR (21h-minuit) · 25%</th>
                        <th className="pb-2 pr-3">DIM JOUR (minuit-21h) · 25%</th>
                        <th className="pb-2 pr-3">DIM SOIR (21h-minuit) · 50%</th>
