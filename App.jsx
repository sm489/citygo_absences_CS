    ...
    for (const m of monthsToProcess) {
      ...
      for (let day = 1; day <= daysInMonth; day++) {
        ...
        const ticketsToday = (prenom, intervals) => {
          ...
          let count = 0;
          // Déjeuner : dès qu'un créneau chevauche, même partiellement, la fenêtre 13h-14h.
          if (merged.some(([s, e]) => s < 14 * 60 && e > 13 * 60)) count += 1;
          // Dîner : dès qu'un créneau chevauche, même partiellement, la fenêtre 19h-20h.
          if (merged.some(([s, e]) => s < 20 * 60 && e > 19 * 60)) count += 1;
          return count;
        };
