...
export default function AbsenceTracker() {
  ...
  useEffect(() => {
    (async () => {
      ...
      // Supabase la reconnecte automatiquement mais on la garde volontairement sur
      // l'écran "nouveau mot de passe" plutôt que de la laisser entrer directement.
      // On ne vérifie ceci qu'UNE SEULE FOIS au chargement, puis on nettoie l'adresse :
      // sinon, cette trace resterait dans l'URL et referait perdre toutes les connexions
      // suivantes (bug corrigé).
    ...
    const subscription = onAuthStateChange(async (session, event) => {
      // Pendant le parcours "mot de passe oublié", Supabase déclenche une connexion
      // automatique (événement PASSWORD_RECOVERY) : on l'ignore volontairement pour
      // rester sur l'écran de saisie du nouveau mot de passe. On ne se base plus sur
      // l'adresse du navigateur ici (elle peut garder une trace périmée), uniquement
      // sur l'événement transmis par Supabase.
              ...
              <div className="mt-3 flex items-center justify-end">
                {!canEditPlanning && (
                  <p className="text-xs text-slate-400 mr-3">
                    Seuls les administrateurs peuvent modifier le planning.
                  </p>
                )}
                <button
                  type="button"
