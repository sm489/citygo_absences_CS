...
export default function AbsenceTracker() {
  ...
  async function handleAddShift() {
    ...
    try {
      await addShift({ prenom: planPrenom.trim(), jour: planJour, heureDebut: planHeureDebut, heureFin: planHeureFin });
  ...
  async function saveEditShift(id) {
    ...
    try {
      await updateShift(id, { prenom: p.trim(), jour: j, heureDebut: hd, heureFin: hf });
      await refreshPlanning();
    } catch (e) {
  ...
  async function handleSubmit() {
    ...
    setSaving(true);
    try {
      await addAbsence({ prenom: prenom.trim(), date, heureDebut, heureFin });
  ...
  async function saveEdit(id) {
    ...
    try {
      await updateAbsence(id, { prenom: p.trim(), date: d, heureDebut: hd, heureFin: hf });
      await refreshEntries();
    } catch (e) {
  ...
  async function confirmVolunteer(id) {
    ...
    try {
      await addVolunteer(id, { prenom: nom, heureDebut: hd, heureFin: hf });
      await refreshEntries();
    } catch (e) {
      console.error("Erreur de positionnement:", e);
  ...
  function handleExportWeeklyHoursPDF() {
    const rows = weeklyHoursByPerson.map((row) => [
      ...
      formatDuration(row.monthlyMinutes + row.extraMinutes),
    ]);
    exportTableToPDF(
      "Heures travaillées par personne",
      ["Personne", "Heures contractuelles", "Heures supplémentaires", "Total"],
      rows,
