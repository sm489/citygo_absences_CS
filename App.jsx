...
export default function AbsenceTracker() {
  ...
  const [archiveMonthFilter, setArchiveMonthFilter] = useState(today.getMonth() + 1);
  const [freelanceYearFilter, setFreelanceYearFilter] = useState(today.getFullYear());
  const [freelanceMonthFilter, setFreelanceMonthFilter] = useState(today.getMonth() + 1);
  ...
  const archivedFilteredSorted = useMemo(
    () =>
      archivedSorted.filter((e) => {
        ...
        return Number(y) === archiveYearFilter && Number(m) === archiveMonthFilter;
      }),
  ...
  const freelanceActiveSorted = useMemo(
    () => freelanceSorted.filter((a) => !isPast(a)),
    [freelanceSorted]
  );
  const freelanceArchivedSorted = useMemo(
    () =>
      freelanceSorted
        .filter((a) => isPast(a))
  ...
  const freelanceActiveFilteredSorted = useMemo(
    () =>
      freelanceActiveSorted.filter((a) => {
        const [y, m] = a.date.split("-");
        return Number(y) === freelanceYearFilter && Number(m) === freelanceMonthFilter;
      }),
    [freelanceActiveSorted, freelanceYearFilter, freelanceMonthFilter]
  );
  const freelanceArchivedFilteredSorted = useMemo(
    () =>
      freelanceArchivedSorted.filter((a) => {
        const [y, m] = a.date.split("-");
        return Number(y) === archiveYearFilter && Number(m) === archiveMonthFilter;
      }),
    [freelanceArchivedSorted, archiveYearFilter, archiveMonthFilter]
  );
    ...
    >
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-10 sm:py-14">
        ...
        {page === "admin" ? (
            ...
            <section className="bg-white border border-slate-200 rounded-2xl shadow-sm p-5 sm:p-6">
              <div className="flex items-center justify-between flex-wrap gap-2 mb-3">
                <p className="flex items-center gap-1.5 text-sm font-semibold text-slate-600 uppercase tracking-wide">
                  ...
                  {freelanceAbsences ? `(${freelanceActiveFilteredSorted.length})` : ""}
                </p>
                <div className="flex items-center gap-2">
                  <select
                    value={freelanceMonthFilter}
                    onChange={(ev) => setFreelanceMonthFilter(Number(ev.target.value))}
                    className="text-xs font-medium px-2.5 py-1.5 rounded-lg border border-slate-200 text-slate-600 focus:outline-none focus:ring-2 focus:ring-violet-500"
                  >
                    {MONTH_NAMES.map((label, i) => (
                      <option key={label} value={i + 1}>
              ...
              {freelanceAbsences === null ? (
              ...
              ) : freelanceActiveFilteredSorted.length === 0 ? (
                ...
                <ul className="space-y-2">
                  {freelanceActiveFilteredSorted.map((a) => (
                    <li
                      key={a.id}
            ...
            <div className="flex items-center justify-between flex-wrap gap-2 mb-4">
              ...
              <div className="flex items-center gap-2">
                <select
                  ...
                  onChange={(ev) => setArchiveMonthFilter(Number(ev.target.value))}
                  className="text-xs font-medium px-2.5 py-1.5 rounded-lg border border-slate-200 text-slate-600 focus:outline-none focus:ring-2 focus:ring-violet-500"
                >
                  {MONTH_NAMES.map((label, i) => (
                    <option key={label} value={i + 1}>
          ...
          <section className="bg-white border border-slate-200 rounded-2xl shadow-sm p-5 sm:p-6 mb-8">
            <p className="flex items-center gap-1.5 text-sm font-semibold text-slate-600 uppercase tracking-wide mb-4">
              <Archive className="w-4 h-4" /> Absences archivées (Freelances){" "}
              {freelanceArchivedFilteredSorted.length ? `(${freelanceArchivedFilteredSorted.length})` : ""}
            </p>
            {freelanceArchivedFilteredSorted.length === 0 ? (
              <div className="text-center py-10 text-slate-400 text-sm">
                Aucune absence de freelance archivée pour cette période.
              </div>
            ) : (
              <ul className="space-y-2">
                {freelanceArchivedFilteredSorted.map((a) => (
                  <li
                    key={a.id}
                    className="border border-slate-100 rounded-xl p-3 flex items-start justify-between gap-3 opacity-80"
                  >
