...
const FREELANCE_NAMES = new Set(["Aurore", "Yann", "Gabrielle", "Hamza", "Corolla", "Khalid"]);
...
// Affiche "(Free)" à côté du prénom des personnes en contrat freelance, au lieu de la couleur
// violette utilisée auparavant (elles s'affichent maintenant en noir, comme tout le monde).
function displayPersonName(prenom) {
  return FREELANCE_NAMES.has(prenom) ? `${prenom} (Free)` : prenom;
}
...
export default function AbsenceTracker() {
  ...
  async function handleAddTeamMember() {
    ...
    if (teamRoster.some((n) => normalizeName(n) === normalizeName(resolved))) {
  ...
  async function handleAddFreelanceAbsence() {
    ...
    if (!FREELANCE_NAMES.has(resolvedFreelancePrenom)) {
  ...
  const knownNamesNormalized = useMemo(
    () => new Set([...teamMembers, ...teamRoster, "Ghost"].map(normalizeName)),
  ...
  function resolveKnownName(raw) {
    ...
    for (const known of [...teamMembers, ...teamRoster, "Ghost"]) {
  ...
  const weeklyHoursByPerson = useMemo(() => {
    ...
    const allNames = [...new Set([...teamMembers, ...teamRoster])];
              ...
              {weeklyHoursByPerson.length === 0 ? (
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-sm text-center">
                    ...
                    <tbody>
                      {weeklyHoursByPerson
                        .filter((row) => teamRoster.includes(row.prenom) && row.prenom !== "Capucine")
                        .sort((a, b) => {
                          const order = teamRoster.filter((n) => n !== "Capucine");
