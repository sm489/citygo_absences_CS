...
export default function AbsenceTracker() {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        const res = await window.storage.get("hours-adjustments", true);
        setHoursAdjustments(res && res.value ? JSON.parse(res.value) : {});
      } catch (e) {
        setHoursAdjustments({});
      }
    })();
  ...
  function updateHoursAdjustment(prenom, field, value) {
    const key = hoursAdjustmentKeyFor(prenom);
    ...
    setHoursAdjustments((prev) => {
      ...
      if (window.storage) {
        window.storage.set("hours-adjustments", JSON.stringify(next), true).catch(() => {});
      }
      return next;
    });
  }
  ...
  useEffect(() => {
    (async () => {
      if (!window.storage) return;
      try {
        const res = await window.storage.get("ticket-adjustments", true);
        setTicketAdjustments(res && res.value ? JSON.parse(res.value) : {});
      } catch (e) {
  ...
  function updateTicketAdjustment(prenom, field, value) {
    const key = adjustmentKeyFor(prenom);
    ...
    setTicketAdjustments((prev) => {
      ...
      if (window.storage) {
        window.storage.set("ticket-adjustments", JSON.stringify(next), true).catch(() => {});
      }
      return next;
    });
  }
  ...
  useEffect(() => {
    (async () => {
      if (!window.storage) {
        console.error("window.storage indisponible");
        setPlanning([]);
        return;
      }
      ...
      let parsed = [];
      try {
        const res = await window.storage.get("team-planning", true);
        parsed = res && res.value ? JSON.parse(res.value) : [];
      } catch (e) {
        // La clé n'existe pas encore (première utilisation) : ce n'est pas une vraie erreur.
        parsed = [];
      }

      const base = parsed.length > 0 ? parsed : SEED_PLANNING;
      const merged = fixClemenceShifts(mergeFullDayShifts(base));
      setPlanning(merged);

      // Si la fusion/correction a changé quelque chose (ou si on vient de semer les données), on persiste.
      if (JSON.stringify(merged) !== JSON.stringify(base)) {
        try {
          await window.storage.set("team-planning", JSON.stringify(merged), true);
        } catch (e) {
          console.error("Erreur de sauvegarde du planning:", e);
        }
      }
    })();
  }, []);
  ...
  async function persistPlanning(next) {
    ...
    try {
      if (!window.storage) throw new Error("stockage indisponible");
      const ok = await window.storage.set("team-planning", JSON.stringify(next), true);
      if (!ok) throw new Error("échec");
      setPlanningError("");
    } catch (e) {
      console.error("Erreur de sauvegarde du planning:", e);
      setPlanningError("Affiché ci-dessous, mais la synchronisation partagée a échoué.");
    } finally {
  ...
  useEffect(() => {
    (async () => {
      try {
        if (!window.storage) throw new Error("window.storage indisponible");
        const res = await window.storage.get("team-exchanges", true);
        setExchanges(res && res.value ? JSON.parse(res.value) : []);
      } catch (e) {
        setExchanges([]);
      }
    })();
  ...
  async function persistExchanges(next) {
    setExchanges(next);
    setExchangesSaving(true);
    try {
      if (!window.storage) throw new Error("stockage indisponible");
      const ok = await window.storage.set("team-exchanges", JSON.stringify(next), true);
      if (!ok) throw new Error("échec");
      setExchangesError("");
    } catch (e) {
      console.error("Erreur de sauvegarde des échanges:", e);
      setExchangesError("Affiché ci-dessous, mais la synchronisation partagée a échoué.");
    } finally {
  ...
  function handleAddExchange() {
    ...
    const exchange = {
      ...
      prenom: exPrenom.trim(),
      date: exDate,
      heureDebut: exHeureDebut,
      heureFin: exHeureFin,
      collegue: hasCollegue ? exCollegue.trim() : "",
    ...
    persistExchanges([exchange, ...(exchanges || [])]);
    setExPrenom("");
    setExDate("");
    setExHeureDebut("");
    setExHeureFin("");
  ...
  function submitExchangePosition(id) {
  ...
  function approveExchange(id) {
    persistExchanges((exchanges || []).map((ex) => (ex.id === id ? { ...ex, status: "approved" } : ex)));
  }

  function declineExchange(id) {
    persistExchanges((exchanges || []).map((ex) => (ex.id === id ? { ...ex, status: "rejected" } : ex)));
  }

  function handleDeleteExchange(id) {
    persistExchanges((exchanges || []).filter((ex) => ex.id !== id));
  }
  ...
  function handleAddShift() {
    ...
    const shift = {
      id: `${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
      prenom: planPrenom.trim(),
      jour: planJour,
      heureDebut: planHeureDebut,
      heureFin: planHeureFin,
    };
    persistPlanning([...(planning || []), shift]);
    setPlanPrenom("");
    setPlanJour("");
    setPlanHeureDebut("");
    setPlanHeureFin("");
  ...
  function handleDeleteShift(id) {
    persistPlanning((planning || []).filter((s) => s.id !== id));
    if (planEditingId === id) {
  ...
  function saveEditShift(id) {
    ...
    const next = (planning || []).map((s) =>
      s.id === id ? { ...s, prenom: p.trim(), jour: j, heureDebut: hd, heureFin: hf } : s
    );
  ...
  useEffect(() => {
    (async () => {
      try {
        if (!window.storage) throw new Error("window.storage indisponible");
        const res = await window.storage.get("absences-list", true);
  ...
  async function persist(next) {
    // Mise à jour immédiate de l'écran, sans attendre la sauvegarde
    setEntries(next);
    setSaving(true);
    try {
      if (!window.storage) throw new Error("stockage indisponible");
      const ok = await window.storage.set("absences-list", JSON.stringify(next), true);
      if (!ok) throw new Error("échec");
      setError("");
    } catch (e) {
      console.error("Erreur de sauvegarde:", e);
      setError("Affiché ci-dessous, mais la synchronisation partagée a échoué : les autres personnes ne verront peut-être pas cette mise à jour.");
    } finally {
      setSaving(false);
    }
  }
  ...
  function saveEdit(id) {
    ...
    const next = (entries || []).map((e) =>
      e.id === id ? { ...e, prenom: p.trim(), date: d, heureDebut: hd, heureFin: hf } : e
    );
    persist(next);
    setEditingId(null);
    setEditError("");
  ...
  function confirmVolunteer(id) {
    ...
    const next = (entries || []).map((e) =>
      e.id === id ? { ...e, volunteers: [...e.volunteers, volunteer] } : e
    );
    persist(next);
  ...
  function approveVolunteer(entryId, volunteerId) {
    const next = (entries || []).map((e) => {
      if (e.id !== entryId) return e;

      const volunteersAfterApproval = e.volunteers.map((v) =>
        v.id === volunteerId ? { ...v, status: "approved" } : v
      );

      const approved = volunteersAfterApproval.filter((v) => v.status === "approved" || !v.status);
      const isNowComplete = getUncoveredRanges(e, approved).length === 0;

      const finalVolunteers = isNowComplete
        ? volunteersAfterApproval.map((v) =>
            v.status === "pending" ? { ...v, status: "rejected" } : v
          )
        : volunteersAfterApproval;

      return { ...e, volunteers: finalVolunteers };
    });
    persist(next);
  }

  function declineVolunteer(entryId, volunteerId) {
    const next = (entries || []).map((e) =>
      e.id === entryId
        ? {
            ...e,
  ...
  function removeVolunteerEntry(entryId, volunteerId) {
    const next = (entries || []).map((e) =>
      e.id === entryId
        ? { ...e, volunteers: e.volunteers.filter((v) => v.id !== volunteerId) }
        : e
    );
    persist(next);
  }
  ...
  const knownNamesNormalized = useMemo(
    () => new Set([...teamMembers, "Camille"].map(normalizeName)),
    [teamMembers]
  );
  ...
  const weeklyHoursByPerson = useMemo(() => {
    ...
    const BREAK_1H_NAMES = new Set(["Capucine", "Sylvain", "Clémence"]);
  ...
  }, [teamMembers, planningByDay, hoursYearFilter, hoursMonthFilter, extraMinutesByPersonMonth]);
  ...
  // Tickets restaurant du mois sélectionné : on part du planning récurrent de chaque jour,
    ...
    for (const m of monthsToProcess) {
      ...
      for (let day = 1; day <= daysInMonth; day++) {
        ...
        const ticketsToday = (prenom, intervals) => {
          ...
          let count = 0;
          // Déjeuner : il faut avoir commencé avant 13h ET travailler encore à 14h (une fois les
          // créneaux du jour fusionnés), sinon on considère que la personne a pu manger avant de
          // prendre son poste.
          if (merged.some(([s, e]) => s < 13 * 60 && e >= 14 * 60)) count += 1;
          // Dîner : il faut couvrir tout le créneau 19h-20h (peu importe si ça vient d'un seul
          // créneau ou de plusieurs créneaux du même jour qui se rejoignent), sinon on considère
          // que la personne peut manger après sa fin de poste (si elle finit à 19h ou avant).
          if (merged.some(([s, e]) => s <= 19 * 60 && e >= 20 * 60)) count += 1;
          return count;
        };
    ...
    >
      ...
      <aside className="hidden sm:flex sm:flex-col w-60 shrink-0 min-h-screen bg-white border-r border-slate-200 p-5">
        ...
        <div className="mt-auto pt-4 border-t border-slate-100">
          ...
          <p className="text-sm font-medium text-slate-900 mb-1.5">{authUser?.email}</p>
                ...
                <div className="relative flex items-center gap-1.5">
                  ...
                  {showHoursInfo && (
                    <div className="absolute top-6 left-0 z-10 w-80 bg-white border border-slate-200 rounded-xl shadow-lg p-4 normal-case">
                      ...
                      <p className="text-xs text-slate-500 leading-relaxed">
                        Pour Clémence, Sylvain et Capucine, une heure de pause est automatiquement décomptée
                        par jour travaillé.
                      </p>
                    </div>
                  )}
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {weeklyHoursByPerson
                        ...
                        const totalMinutes =
                          row.monthlyMinutes + row.extraMinutes + adj.plus * 60 - adj.minus * 60;
                        return (
            ...
            <section className="bg-white border border-slate-200 rounded-2xl shadow-sm p-5 sm:p-6">
              ...
              {personSummaryTable.length === 0 ? (
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="text-left text-[10px] font-semibold text-slate-300 uppercase tracking-wide">
                        <th></th>
                        <th></th>
            ...
            <section className="bg-white border border-slate-200 rounded-2xl shadow-sm p-5 sm:p-6">
              ...
              <div className="grid grid-cols-6 gap-3 sm:gap-4">
                <div className="col-span-2">
                  ...
                  />
                  {isUnknownName(planPrenom) && (
                    <p className="text-xs text-amber-600 mt-1">Prénom inconnu de l'équipe, vérifiez l'orthographe.</p>
                  )}
                </div>
                <div className="col-span-2">
                          ...
                          <li key={ex.id} className="bg-white border border-slate-200 rounded-xl p-4">
                            ...
                            {ex.status === "open" && exPositionOpenFor !== ex.id && (
                              <div className="mt-6 pt-3 border-t border-slate-100 flex justify-center">
                                <button
                                  onClick={() => setExPositionOpenFor(ex.id)}
        ...
        <section>
          <div className="flex items-center justify-between flex-wrap gap-3 mb-3">
            <h2 className="flex items-center gap-1.5 text-sm font-semibold text-slate-500 uppercase tracking-wide">
              <Calendar className="w-4 h-4" /> Absences renseignées {entries ? `(${entries.length})` : ""}
            </h2>
                ...
                return (
                  ...
                  >
                    ...
                    {!past && editingId !== e.id && openVolunteerFor !== e.id && (
                      <div className="mt-6 pt-3 border-t border-slate-100 flex justify-center">
                        <button
                          onClick={() => !isComplete && setOpenVolunteerFor(e.id)}
                          disabled={isComplete}
