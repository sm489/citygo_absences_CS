...
// Création de compte. Le domaine @citygo.me est vérifié ici, mais vous pouvez
// aussi le forcer côté Supabase (Authentication → Providers → Email → "Allow
// only these domains") pour une double sécurité.
export async function signUp(email, password) {
  if (!email.toLowerCase().endsWith("@citygo.me")) {
    return { error: "Utilisez une adresse email @citygo.me." };
  }
...
// Appelé sur la page /reset-password, une fois que la personne a cliqué sur
...
export async function getCurrentProfile() {
  ...
  return { email: data.email, isAdmin: data.role === "admin" };
}
