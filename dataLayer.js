...
export async function addVolunteer(absenceId, { prenom, heureDebut, heureFin, email, date }) {
  const { error } = await supabase
...
// Ajustements manuels pour la carte "Heures supplémentaires par personne" (distincts de
// hours_adjustments, qui sert à la carte "Heures travaillées par personne").
export async function loadExtraAdjustments() {
  const { data, error } = await supabase.from("extra_adjustments").select("*");
  if (error) throw error;
  const map = {};
  for (const row of data) map[`${row.prenom}|${row.period_key}`] = { plus: row.plus, minus: row.minus };
  return map;
...
export async function setExtraAdjustment(prenom, periodKey, field, value) {
  const { error } = await supabase
    .from("extra_adjustments")
    .upsert({ prenom, period_key: periodKey, [field]: value }, { onConflict: "prenom,period_key" });
  if (error) throw error;
}

// ---------------------------------------------------------------------------
// RÉPERTOIRE DES ABSENCES (Freelances) — simple liste, sans système de
// remplacement/validation, distincte de la table "absences" classique.
// ---------------------------------------------------------------------------

export async function loadFreelanceAbsences() {
  const { data, error } = await supabase
    .from("freelance_absences")
    .select("*")
    .order("date", { ascending: true });
  if (error) throw error;
  return data.map((a) => ({
...
export async function addFreelanceAbsence({ prenom, date, heureDebut, heureFin }) {
  const { data, error } = await supabase
    .from("freelance_absences")
    .insert({ prenom, date, heure_debut: heureDebut, heure_fin: heureFin })
    .select()
...
export async function deleteFreelanceAbsence(id) {
  const { error } = await supabase.from("freelance_absences").delete().eq("id", id);
  if (error) throw error;
}

// ---------------------------------------------------------------------------
// LISTE DE L'ÉQUIPE (roster modifiable, remplace une liste figée dans le code) —
// utilisée par "Heures travaillées par personne" (Administration) et par les
// suggestions de saisie du prénom dans Planning et Absences & Remplacements.
// ---------------------------------------------------------------------------

export async function loadTeamRoster() {
  const { data, error } = await supabase.from("team_roster").select("*").order("prenom", { ascending: true });
  if (error) throw error;
  return data.map((r) => ({ prenom: r.prenom, isFreelance: !!r.is_freelance }));
}

export async function setTeamMemberFreelance(prenom, isFreelance) {
  const { error } = await supabase.from("team_roster").update({ is_freelance: isFreelance }).eq("prenom", prenom);
  if (error) throw error;
}

export async function addTeamMember(prenom) {
  const { error } = await supabase.from("team_roster").insert({ prenom });
  if (error) throw error;
}

export async function deleteShiftsByPrenom(prenom) {
  const { error } = await supabase.from("planning_shifts").delete().eq("prenom", prenom);
  if (error) throw error;
}

export async function removeTeamMember(prenom) {
  await deleteShiftsByPrenom(prenom);
  const { error } = await supabase.from("team_roster").delete().eq("prenom", prenom);
  if (error) throw error;
}
